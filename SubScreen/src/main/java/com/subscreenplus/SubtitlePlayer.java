...
import com.subscreenplus.Subtitles.SMIFormat;
import com.subscreenplus.Subtitles.SrtFormat;
...
import com.subscreenplus.Subtitles.TutorialFormat;
...
public class SubtitlePlayer {
	...
	public void main(TextView toEdit, Context _context, BufferedInputStream fileData, Activity activity, String encoding, String _fileName) {
        context = _context;
        fileName = _fileName;
        playString = context.getString(com.subscreenplus.R.string.begin_play);
        ...
        if (subFile == null){
            parentActivity.displayBackMessage(
                    context.getString(com.subscreenplus.R.string.bad_format_message), context.getString(com.subscreenplus.R.string.bad_format_title));
            return;
        }
		Typeface test_font = Typeface.createFromAsset(context.getResources().getAssets(),"DejaVuSans.ttf");
		toEdit.setTypeface(test_font);
		outputTo = new AndroidOutput(activity,context.getResources().getDimension(com.subscreenplus.R.dimen.activity_text_size));
        ...
        try {
            ...
            parentActivity.displayBackMessage(
                    context.getText(com.subscreenplus.R.string.bad_format_message).toString(),"Sorry");
            return;
        }
        if (smiSub != null) {
	...
	private void playSubtitles(ArrayList<TextBlock> blocks, Output outputTo) {
        ...
        outputTo.outputText(context.getString(com.subscreenplus.R.string.finish_play));
        finished = true;
        try {
